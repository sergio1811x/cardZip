...
### Обновление по текущей сессии

- Добавлен canonical facts layer: [buildProductFactSheet()](../src/core/factSheet.ts:111)
- Добавлен capability/policy layer: [buildCategoryPolicyProfile()](../src/core/categoryPolicyRegistry.ts:17)
- Добавлен gap planner: [runGapPlanner()](../src/providers/gapPlanner.ts:56)
- Добавлен consistency auditor: [runConsistencyAuditor()](../src/providers/consistencyAuditor.ts:56)
- Добавлен orchestration layer ранних ролей: [runProductRolePipeline()](../src/providers/productRolePipeline.ts:11)
- requiredEdits из consistency auditor встроены в repair loop в [api/step5-qa.ts](../api/step5-qa.ts:354)
- Введены quality metrics payload/summary:
  - [buildQualityMetricsPayload()](../src/core/qualityMetrics.ts:17)
  - [summarizeQualityMetrics()](../src/core/qualityMetrics.ts:31)
- Полный `tsc --noEmit` снова зелёный после правки [tsconfig.json](../tsconfig.json:2)

### Что ещё осталось

1. Внутренне распилить монолит [canonicalizeProduct()](../src/providers/productCanonicalizer.ts:1223) на несколько независимых LLM-role providers, а не только оркестрировать его снаружи.
2. Добавить persistence/дашборд по quality metrics, чтобы видеть fail-rate/warn-rate по этапам в админке и аналитике.
3. Расширить покрытие тестами на полный role-pipeline и на деградационные сценарии моделей.
