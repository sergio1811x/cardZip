...
const INTELLIGENCE_PROMPT = `Ты — закупщик 1688. Задача: по фото и данным карточки точно определить товар и собрать единый профиль для закупочного пакета (отчёт, SKU, вопросы поставщику, ТЗ байеру/карго, чек-лист образца, SEO).
...
- Вопросы должны быть под конкретный productKind, без чужих категорий, максимум 10. Каждый вопрос — отдельная тема, без дублей и парафраза одной просьбы дважды.
- НИКОГДА не выдумывай числовые параметры (состав в %, напряжение, объём, размеры, вес, сертификаты). Если параметра нет в переданных данных — спрашивай обобщённо («Подтвердите состав ткани.»), а не «90% нейлон, 10% спандекс». Число повторяй, только если оно дословно есть в данных.
- Русские поля пиши грамотно: правильные падежи после предлогов («для йоги, фитнеса, бега»); перечисление внутри фразы через двоеточие или скобки («размерная сетка (обхват талии, бёдер, длина по каждому размеру)»), не выбрасывай пунктуацию; буква ё где нужно (бёдер).
- Для кухонной сушилки/стеллажа спрашивай ярусы, размеры 43/53, материал/покрытие, поддон, комплектацию, устойчивость, упаковку.
- Электротовары: SKU вида "美规/欧规/英规/日规/韩规/澳规/国标" — это стандарт вилки (US/EU/UK/JP/KR/AU/CN), а не цвет. Напряжение/мощность/вилку не утверждай как факт без источника — клади в claimedFeatures/unconfirmedFeatures.
- Не приклеивай число к fallback-тексту; если значения нет — null или "не указано" без цифры.
...
export async function generateProductIntelligence(raw: {
  ...
  const systemMsg = 'Ты — закупщик-аналитик товаров с 1688. Определяешь товар и собираешь профиль для закупочного пакета. Отвечай только валидным JSON-объектом, без markdown.';
