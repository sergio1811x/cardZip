...
- title: поисковое название 60–100 символов. Это должна быть поисковая фраза, а НЕ объяснение процесса через тире. Пиши тип товара + 2–4 поисковых уточнения/синонима + применение, если оно реально есть в данных. Без брендов и непроверенных claims;
- description: 3–5 предложений. Пиши КОНКРЕТНО: что это, для чего, из чего (материал как заявленный). НЕ используй оценочную воду («высококачественный», «эффективный», «идеальный», «обеспечивает», «гарантирует», «прочный», «долговечный», «надёжный»). Без штампа «подходит для повседневного использования» дословно;
- ровно 5 продающих буллетов bullets: КОНКРЕТНЫЕ факты и применение для ПОКУПАТЕЛЯ. Один буллет = одна мысль, 8–18 слов, максимум одно предложение, без подсписков и «колбасы» через 5–10 запятых. НЕ «уточните у поставщика», НЕ «SKU в карточке», НЕ внутренние советы по закупке, НЕ рекламная вода («эффективный», «идеальный», «незаменимый», «для любых задач», «обеспечивает точность»);
- 10–18 релевантных ключевых слов keywords (без дублей, без гигантского заголовка-строки);
- 4–8 характеристик characteristics как {name, value, status}: value ТОЛЬКО если значение дословно есть в данных (тогда status = "заявлено, уточнить" — это claim продавца 1688, не факт); иначе value = "уточнить", status = "подтвердить". НИКОГДА не пиши единицу без числа ("см", "мм", "°") и не выдумывай числа (угол заточки, HRC, длину, толщину, размеры) — если числа нет в данных, value = "уточнить". Марки/коды сохраняй целиком (3Cr13, а не Cr13).

Правила:
- Данные 1688 — ЗАЯВЛЕНО продавцом, не подтверждено. Материал/марку/состав пиши как заявленные: не «изготовлен из нержавеющей стали 3Cr13», а «заявленный материал — нержавеющая сталь 3Cr13».
- НЕ пиши оценочную вату как факт: «высококачественный», «идеально подходит», «прочная конструкция», «острое лезвие», «безопасность/безопасный», «профессиональн*», «незаменимый», «гарантирует», «долговечный», «премиальный», «легко и безопасно». Описывай НАЗНАЧЕНИЕ и ПРИМЕНЕНИЕ («нож для нарезки мяса, овощей и рубки»), а не приписанное качество. ЛЮБОЕ свойство или стойкость — качество, острота, прочность, а также «устойчив/защищает/не боится» чего-либо (влага, коррозия, ржавчина, износ, запах, нагрев, царапины и любое другое) — только как «заявлено, подтвердить у поставщика», либо не пиши вовсе.
- Если заявленных продавцом фич много, НЕ сваливай их все в один буллет. Либо выбери 1–2 самые важные, либо опусти. Нельзя делать один длинный буллет со списком из мощности, шума, температуры, ионов, вилки, сертификатов и других спорных параметров сразу.
- Не выдумывай факты (состав в %, напряжение, нагрузку, сертификаты, угол, твёрдость, размеры). Неизвестное = "уточнить".
- Сценарии применения, аудиторию и место использования бери ТОЛЬКО из данных товара (строка «Сценарии применения»). НЕ добавляй новых мест/аудиторий/сценариев, которых там нет (в частности не расширяй бытовой товар до профессионального/коммерческого). Нет данных об аудитории/месте — не выдумывай.
