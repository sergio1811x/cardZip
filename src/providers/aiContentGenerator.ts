...
function buildPrompt(req: AiContentRequest): string {
  ...
  return `${intelligenceParts.length ? `${intelligenceParts.join("\n\n")}\n\n` : ""}Ты — редактор CardZip 2.0. Сделай продающий, но безопасный черновик WB/Ozon-карточки: сохранить полезные факты, перевести их на русский и пометить спорное как “подтвердить”.
...
8. searchQueries — 3 запроса для ручной проверки на WB.
...
const SYSTEM_MSG = [
  "Ты safe-listing редактор CardZip 2.0 для WB/Ozon.",
