...
const DEFAULT_OPENROUTER_MODELS = [
  ...
  process.env.AI_TEXT_PRIMARY_MODEL ?? "deepseek/deepseek-v4-pro",
  process.env.AI_TEXT_FALLBACK_MODEL_1 ?? "google/gemini-3.1-flash-lite",
  process.env.AI_TEXT_FALLBACK_MODEL_2 ?? "google/gemini-2.5-flash",
  process.env.AI_TEXT_FALLBACK_MODEL_3 ?? "deepseek/deepseek-v4-flash",
].filter(Boolean);
...
- НИКОГДА не выдумывай числовые параметры (состав в %, напряжение, объём, размеры, вес, сертификаты). Если параметра нет в данных выше — не пиши число: спрашивай обобщённо в supplierQuestions («Подтвердите состав ткани.»), а не «90% нейлон / 10% спандекс». Число указывай, только если оно дословно есть в данных товара.
- Грамотный русский: правильные падежи после предлогов («для йоги, фитнеса, бега», а не «для йога, фитнес, бег»); перечисление внутри фразы оформляй через двоеточие или скобки, не выбрасывай пунктуацию; буква ё где нужно (бёдер).
- description не начинай с голого существительного-обрывка («шорты.») — только полное предложение. bullets — законченные грамотные фразы, не обрывки-слаги.
- В supplierQuestions не повторяй одну и ту же просьбу дважды: каждый вопрос — отдельная тема.
...
async function callProvider(
  ...
  try {
    const res = await fetch(`${baseUrl}/chat/completions`, {
      ...
      body: JSON.stringify({
        model,
        max_tokens: Number(process.env.AI_TEXT_MAX_TOKENS ?? 5000),
