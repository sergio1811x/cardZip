...
${req.riskFlags.isElectrical ? '- Электротовар: не пиши "безопасный", не утверждай про сертификаты\n' : ''}${req.riskFlags.isChildren ? '- Детский товар: не пиши "для детей" без подтверждения\n' : ''}${req.riskFlags.isCosmetic ? '- Косметика: не пиши про состав, который не подтверждён\n' : ''}${req.riskFlags.isFood ? '- Пищевой товар: не пиши про полезные свойства\n' : ''}${req.riskFlags.isMedical ? '- Медицинский товар: не пиши про лечебные свойства\n' : ''}`
  ...
  return `Ты — SEO-копирайтер для маркетплейса Wildberries. Создай контент для карточки товара.

КОНТЕКСТ:
Селлер закупает товар в Китае (1688) и продаёт на Wildberries в России.
...
- НЕ добавляй маркетинговые обещания ("гарантия качества", "высокое качество" и т.п.)
- НЕ включай B2B-платформы (eBay, Amazon, Wish, AliExpress, Lazada)
- НЕ включай внутренние метки 1688 и маркетинговые фразы поставщика без характеристик
- Если данных о свойстве нет — НЕ упоминай его

ЗАДАЧИ:
1. titleRu — коммерческое название для WB (категория + ключевые свойства), до 200 символов${req.brand ? '\n2. titleRuBranded — оригинальное обозначение поставщика (бренд + модель)' : ''}
3. description — SEO-описание до 1200 символов, только подтверждённые факты
4. bullets — 5 коротких тезисов (5-10 слов), начинаются с эмодзи, ТОЛЬКО подтверждённые преимущества
