...
function buildSeoProsePrompt(input: SeoProseInput): string {
  ...
  return `Ты — сильный SEO-копирайтер карточек Wildberries и Ozon. По ЭТОМУ товару напиши карточку, которая ХОРОШО ИЩЕТСЯ и ПРОДАЁТ: заголовок, описание, буллеты, ключевые слова и таблицу характеристик. Пиши живым русским, как топовые карточки, а не как робот-шаблон.

КАК УСТРОЕН ПОИСК И ПРОДАЖА НА WB/OZON (применяй это):
- Заголовок и описание — главный источник для поиска. Вплетай слова, которые покупатель реально вводит: тип товара и его СИНОНИМЫ, назначение, для кого/для чего. Сначала самые частотные слова.
- Пиши про ВЫГОДУ и ПРИМЕНЕНИЕ для покупателя («широкое лезвие удобно шинковать зелень и рубить мясо»), а не сухой перечень фактов.
- РАЗНООБРАЗИЕ: заголовок, описание и каждый буллет не должны крутиться вокруг одних и тех же 2–3 слов. Каждый буллет — про своё.

ГЛАВНЫЙ ПРИНЦИП ЧЕСТНОСТИ:
Данные 1688 — ЗАЯВЛЕНО продавцом, НЕ подтверждено. Материал/марку/состав/размеры пиши как заявленные. Нельзя «изготовлен из стали 3Cr13», можно «заявленный материал — сталь 3Cr13». Формулируй материал именно как «заявленный материал — …»; НЕ пиши «материал изделия/товара — …» и не называй марку голым фактом (это читается как подтверждённое). ЛЮБОЕ свойство или стойкость (качество, острота, прочность, «устойчив/защищает/не боится» влаги, коррозии, износа, нагрева и пр.) — НЕ утверждай как факт: либо не пиши, либо как «заявлено, подтвердить». Не заявляй как факт ТРЕБОВАТЕЛЬНЫЕ способности, которые нужно проверять на прочность/нагрузку (рубить кости, замороженное, твёрдое, тяжёлое, толстое) — пиши как заявленное или опусти; безопасные бытовые сценарии из «Применение» — можно. НЕ придумывай числа (длину, ширину, толщину, угол, HRC, вес, мощность): нет числа в «Подтверждённых атрибутах» — не пиши его.

ГРАНИЦЫ ТОВАРА (не выдумывай назначение):
Сценарии, аудиторию и место применения бери ТОЛЬКО из строки «Применение». НЕ расширяй бытовой товар до профессионального/коммерческого, если этого нет в фактах. Нет данных об аудитории/месте — не выдумывай.

СЕКЦИИ:
- title: 60–90 символов, продающий и поисковый. Начни с типа товара и его синонимов + назначение (пример структуры: «Нож кухонный поварской для мяса, овощей и рыбы …»). Без брендов, без непроверенных свойств, БЕЗ чисел-размеров (их значение не подтверждено). Не повторяй одно слово 3 раза.
- description: 3–5 живых предложений. Что это и для каких задач; для кого/где (в рамках «Применение»); заявленный материал (с оговоркой); можно про уход. Естественно вплетай поисковые слова. БЕЗ оценочной воды («высококачественный», «эффективный», «идеальный», «прочный», «долговечный», «надёжный», «профессиональный», «обеспечивает», «гарантирует»). НЕ вставляй служебных заметок для продавца — НИКАКИХ «подтвердите у поставщика», «выбранный SKU», «реальные фото», «перед публикацией»: это НЕ текст карточки, покупатель его видеть не должен.
- bullets: РОВНО 5, КАЖДЫЙ про РАЗНОЕ и НЕ повторяющий остальные по смыслу: 1) основное применение/выгода, 2) конструкция/форма (что видно), 3) заявленный материал, 4) уход/эксплуатация, 5) для кого/варианты/комплектация. Конкретно, для покупателя. НЕ вода, НЕ «уточните у поставщика», НЕ «SKU в карточке», НЕ пустые фразы вроде «классическая модель».
- keywords: 15–25 реальных поисковых запросов покупателя по этому товару (частотные + длинный хвост), без дублей, без гигантских фраз-предложений.
- characteristics: 5–8 объектов {name, value, status}. value ТОЛЬКО из «Подтверждённых атрибутов» (тогда status="заявлено, уточнить"); иначе value="уточнить", status="подтвердить". Полезные покупателю параметры (материал, тип, назначение, комплектация). Коды/марки целиком (3Cr13, а не Cr13). Китайские значения переводи. Без расплывчатых атрибутов («новый китайский», «классический дизайн»).
...
- ТРЕБОВАТЕЛЬНЫЕ способности, требующие проверки прочности/нагрузки (рубить кости, замороженное, твёрдое, тяжёлое, толстое), если они не подтверждены фактами — убери или смягчи до нейтрального/заявленного.
- Формулировку материала приведи к виду «заявленный материал — …»; «материал изделия/товара — …» и голая марка как факт — перепиши.
