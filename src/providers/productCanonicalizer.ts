...
Верни СТРОГО JSON без markdown:
{
  ...
  "procurementProfile": {
    ...
    "domainRules": {
      "productNature": ["сначала классифицируй природу товара — какие сквозные риск-классы к нему применимы. Выбирай ВСЕ подходящие из: mains_powered (питание от сети), battery (аккумулятор/батарейка), heating (нагрев), motorized (мотор), liquid, aerosol, powder, sharp (режущий), mechanism (механизм/складной), skin_contact, food_contact, textile, fragile, oversized, certified_required (нужна разрешительная документация). Если ничего не применимо — [\"none\"]. Это поле УПРАВЛЯЕТ полем criticalConfirmations."],
      "criticalConfirmations": ["ОБЯЗАТЕЛЬНОЕ, НЕПУСТОЕ поле (пусто ТОЛЬКО если productNature=[\"none\"]). 5–9 сквозных критичных подтверждений, ВЫВЕДЕННЫХ из productNature, — единый доменный блок, который попадёт В ВОПРОСЫ ПОСТАВЩИКУ, В КАРГО И В ТЗ БАЙЕРУ одновременно. ФОРМА: каждый пункт — ЗАКОНЧЕННЫЙ, естественный ВОПРОС поставщику (с вопросительным знаком), а НЕ ярлык. Плохо: «Рабочее напряжение и частота». Хорошо: «Какое рабочее напряжение и частота питания у выбранного SKU?». Для КАЖДОГО риск-класса из productNature добавь его обязательные подтверждения и НЕ пропускай ни одного класса. Логика вывода (адаптируй под товар, не копируй буквально): mains_powered → напряжение И частота, стандарт вилки, реальная мощность, длина шнура, разрешительная документация (CE/RoHS/EAC), фото шильдика; battery → тип/ёмкость и документы для перевозки; heating → макс. температура и защита от перегрева; motorized → тип мотора и подтверждение; sharp → твёрдость/сталь и заточка; food_contact → пищевой сертификат/материал; textile → состав и усадка; certified_required → какие именно документы. mains_powered/heating/battery почти всегда влекут certified_required — не забывай про сертификаты. Всегда добавляй вопрос про ОТСУТСТВИЕ аккумулятора для сетевых приборов, про точную комплектацию выбранного SKU и про количество/вес/габариты мастер-короба. Не выдумывай значения — СПРАШИВАЙ их."],
      "buyerMustCheck": ["6–10 конкретных вопросов поставщику ИМЕННО про этот товар, выведенных из того, что это за предмет: свойства/параметры, о которых спрашивает профессиональный закупщик именно этого объекта (габариты, марка/сорт материала, механизм, безопасность, совместимость). Без дублей, грамотный русский. НЕ ограничивайся ценой/весом/фото. Домкритичные пункты дублируй в criticalConfirmations."],
...
20. Не переноси поля, проверки или claims из похожих, но других товаров. Каждый вопрос и каждое поле должны следовать только из входного товара, SKU, изображения или выявленной productNature.
21. Если данных недостаточно для специализированного поля, не подставляй шаблон: сформулируй нейтральный вопрос на подтверждение или оставь поле пустым.
22. seoAllowedClaims — только буквальные, проверяемые утверждения из входных данных или визуально очевидного. Не записывай сюда выводы по названию товара: назначение, скорость/эффект работы, режимы, удобство, подарочность, комплектацию, способ питания, защиту, тип двигателя и похожие потребительские обещания разрешены только если они прямо подтверждены в источнике. Всё, что требует сертификата, ответа поставщика или проверки образца, включая любое неочевидное применение/выгоду, обязательно клади в seoForbiddenClaims и unconfirmedFeatures.
...
37. domainRules.seo.description — только если есть достаточная доказательная база. Пиши грамотный русский без выдуманных характеристик; если безопасных фактов мало, верни пустую строку, а не маркетинговую догадку.
38. domainRules.seo.sellingBullets — только проверяемые буквальные свойства из входа. НЕ генерируй выгоды, сценарии, ощущения материала или универсальность по названию товара; при недостатке доказательств верни меньше буллетов или пустой список.
39. domainRules.seo.keywords — только поисковые фразы, составленные из подтверждённого типа товара и буквальных безопасных терминов входа. Не добавляй назначение, посадку, эффект, материал, эластичность или аудиторию по догадке.
...
41. domainRules.cargo.sensitiveIssues и whatToRequest должны быть СПЕЦИФИЧНЫ под товар, но только про перевозку: упаковка, фиксация, сжатие, влага, маркировка, грузовые ограничения и транспортные документы. Нельзя помещать сюда QA товара (стирку, посадку, комфорт, эффективность, материал как потребительское свойство). Не выдавай один и тот же обобщённый список для всех товаров и не выдумывай сертификаты/характеристики.
42. domainRules.buyerMustCheck — ОБЯЗАТЕЛЬНОЕ поле и ГЛАВНЫЙ источник вопросов поставщику. Правила:
    ...
    - Самостоятельно выведи нужную схему измерений, составных частей и рисков из конкретного товара; не используй фиксированные списки для категорий.
...
44. СНАЧАЛА заполни productNature (все применимые риск-классы), ПОТОМ выведи criticalConfirmations строго из этих классов — это цепочка рассуждения, а не список наугад. Правило полноты: для каждого класса в productNature в criticalConfirmations ОБЯЗАН быть хотя бы один пункт; если productNature ≠ [\"none\"], criticalConfirmations НЕ может быть пустым. Это сквозной доменный блок — он одинаково важен для вопросов поставщику, карго и ТЗ байеру, поэтому не забывай про разрешительные документы и мастер-короб там, где они уместны по природе товара.
`;
