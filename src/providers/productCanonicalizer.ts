...
Верни СТРОГО JSON без markdown:
{
  ...
  "procurementProfile": {
    ...
    "domainRules": {
      "criticalConfirmations": ["3–8 СКВОЗНЫХ критичных подтверждений, определяемых природой товара, — единый доменный блок, который обязан попасть В ВОПРОСЫ ПОСТАВЩИКУ, В КАРГО И В ТЗ БАЙЕРУ одновременно. Реши по тому, ЧТО это за предмет: если товар ПИТАЕТСЯ ОТ СЕТИ/электроприбор — обязателен полный электроблок (тип вилки; напряжение И частота 220–240V/50–60Hz; реальная мощность; длина сетевого шнура; тип мотора и подтверждение щёточный/бесщёточный; режимы температуры/скорости; подтверждение ОТСУТСТВИЯ аккумулятора; сертификаты CE/RoHS/EAC; фото шильдика с маркировкой питания крупно; количество, вес и габариты мастер-короба). Аналогично: батарея → документы и правила перевозки; текстиль → состав и усадка; контакт с едой → пищевой сертификат. НЕ хардкодь под фен — выведи блок из природы ЭТОГО товара. Пусто, только если товар вообще без домкритичных подтверждений."],
