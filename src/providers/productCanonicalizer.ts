...
Верни СТРОГО JSON без markdown:
{
  ...
  "procurementProfile": {
    ...
    "domainRules": {
      ...
      "criticalConfirmations": ["ОБЯЗАТЕЛЬНОЕ, НЕПУСТОЕ поле (пусто ТОЛЬКО если productNature=[\"none\"]). 4–8 сквозных критичных подтверждений, ВЫВЕДЕННЫХ из productNature, — единый доменный блок, который попадёт В ВОПРОСЫ ПОСТАВЩИКУ, В КАРГО И В ТЗ БАЙЕРУ одновременно. Для КАЖДОГО риск-класса из productNature добавь его обязательные подтверждения и НЕ пропускай ни одного класса. Логика вывода (адаптируй под товар, не копируй буквально): mains_powered → рабочее напряжение И частота, стандарт вилки, реальная мощность, длина шнура, разрешительная документация (CE/RoHS/EAC), фото шильдика с маркировкой питания; battery → тип/ёмкость и документы для перевозки; heating → макс. температура и защита от перегрева; motorized → тип мотора и подтверждение; sharp → твёрдость/сталь и заточка; food_contact → пищевой сертификат/материал; textile → состав и усадка; certified_required → какие именно документы. Всегда добавляй: подтверждение ОТСУТСТВИЯ аккумулятора для сетевых приборов, количество/вес/габариты мастер-короба. Не выдумывай значения — СПРАШИВАЙ их."],
