...
- Если SKU содержит единицу измерения, которая является техническим параметром товара (например, “10кг/10kg/10公斤” у весов = максимальная нагрузка; “5л/5L” у ёмкости = объём; “300мл” у диспенсера = объём), — интерпретируй это как параметр товара в терминах домена, а не как “модель/версию/комплектацию”. Например: “10кг” у весов → “макс. нагрузка 10 кг”. Не называй это “Модель: 10”.
- Примеры finalKind/domainKind для распространённых товаров: kitchen_scale (электронные кухонные весы), food_warmer (подогреватель еды), heating_pad (грелка), juicer (соковыжималка), umbrella (зонт), sleep_mask (маска для сна), footwear (обувь), clothing (одежда), tool_kit (набор инструментов). При kitchen_scale обязательно включи в domainRules: макс. нагрузку, точность/шаг, функцию тарирования, единицы измерения, тип батареек, наличие инструкции. В seoForbiddenClaims запрети: “профессиональные”, “лабораторные”, “медицинские”, “высокая точность без подтверждения”.
