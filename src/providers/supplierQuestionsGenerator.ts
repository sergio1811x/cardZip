...
- ОБЯЗАТЕЛЬНО включи ключевые ИЗМЕРИМЫЕ технические параметры, которые определяют качество и пригодность ИМЕННО этого товара и которые нельзя узнать из карточки. Ты сам определяешь их по типу товара. Примеры логики: режущий инструмент → твёрдость стали (HRC), угол и тип заточки, толщина клинка/обуха; ЭЛЕКТРОПРИБОР → напряжение И частота (220–240V / 50–60Hz), реальная мощность, тип вилки, длина шнура, режимы/скорости, тип мотора и подтверждение (щёточный/бесщёточный), наличие/отсутствие аккумулятора, сертификаты (CE/RoHS/EAC) — для электроприбора спроси ВСЕ эти пункты; ткань/одежда → состав, плотность, усадка; ёмкость → реальный объём, герметичность. Не ограничивайся этими примерами — думай под конкретный товар.
- СОРТИРУЙ вопросы по важности для решения о закупке и образце: самые критичные для качества/пригодности/безопасности — ПЕРВЫМИ; переговорные и второстепенные (оптовая цена, короб, фото) — в конце.
