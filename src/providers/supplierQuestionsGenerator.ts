...
- ОБЯЗАТЕЛЬНО включи ключевые ИЗМЕРИМЫЕ технические параметры, которые определяют качество и пригодность ИМЕННО этого товара и которые нельзя узнать из карточки. Ты сам определяешь их по типу товара. Примеры логики: режущий инструмент → твёрдость стали (HRC), угол и тип заточки, толщина клинка/обуха; электротовар → напряжение, мощность, тип вилки, сертификация; ткань/одежда → состав, плотность, усадка; ёмкость → реальный объём, герметичность. Не ограничивайся этими примерами — думай под конкретный товар.
