...
export function formatRiskMessages(flags: RiskFlags): string[] {
  ...
  if (flags.hasBrand && flags.brand) {
    msgs.push(`Обнаружен бренд ${flags.brand}. Не используйте его в карточке карточке товара без подтверждённых прав на товарный знак.`);
  }
  ...
  if (flags.marketDataUnreliable) {
    msgs.push('Данные карточке товара ограничены. Не используйте автоматическую цену как финальное решение о закупке.');
  }
