...
export function buildConclusion(
  ...
  if (hasAnyWb && !economics.isSyntheticPrice) {
    ...
    if (marginPositive) {
      return {
        platform,
        icon: '🟢',
        headline: 'Сценарий вручную проверен. Подтвердите параметры товара перед образцом.',
        disclaimers,
      };
    }
    return {
      platform,
      icon: '🔴',
      headline: 'Сценарная себестоимость высокая. Уточните оптовую цену и расходы вручную.',
      disclaimers,
    };
  }

  if (noWb) {
    disclaimers.push('Проверьте спрос и конкурентов вручную перед закупкой.');
    return {
      platform,
      icon: '⚪️',
      headline: 'Автоматическая проверка рынка не используется. Проверьте конкурентов вручную перед закупкой.',
      disclaimers,
    };
  }
  ...
  return {
    platform,
    icon: '🟡',
    headline: 'Предварительная себестоимость рассчитана. Продажную цену проверьте вручную.',
    disclaimers,
  };
}
