...
const STAGES: ProgressStage[] = [
  ...
  { key: 'cost', pct: 72, text: 'Считаю cost-only экономику без неподтверждённого ROI' },
...
function getHoldTexts(stage: ProgressStage): string[] {
  ...
  const byKey: Record<string, string[]> = {
    ...
    elim_done: ['Данные 1688 получены, перехожу к AI-разбору', 'Карточка собрана, дальше очищаю и перевожу', 'Готовлю Product Intelligence для закупочного пакета'],
    ...
    cost: ['Считаю закупку в рублях и себестоимость без карго', 'Не считаю ROI без рынка и вашей цены продажи', 'Готовлю экономику честно, без псевдо-точности'],
